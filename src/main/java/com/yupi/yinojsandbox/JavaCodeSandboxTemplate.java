...
import com.yupi.yinojsandbox.utils.MakeCompileErrorResponse;
...
//        2. 编译代码，得到 class 文件
        ...
        log.info("编译报错信息:{}",compileFileExecuteMessage);
//        System.out.println(compileFileExecuteMessage);
        if(compileFileExecuteMessage.getExitValue()!=0) {
           outputResponse = MakeCompileErrorResponse.makeCompileErrorResponse(compileFileExecuteMessage);
        }
        else{
            // 3. 执行代码，得到输出结果
            List<ExecuteMessage> executeMessageList = runFile(userCodeFile, inputList,timeLimit,memoryLimit);
            log.info(executeMessageList.toString());
//        4. 收集整理输出结果
